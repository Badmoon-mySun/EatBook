                                ...
                                <div class="tab-pane fade active in" id="detail">
                                    <table class="table">
                                        <form action="${uri}" method="post" id="edit_register" name="form" class="fontform" onsubmit="return validateForm2()">
                                            ...
                                            <h4 class="htext">Имя<input cols="30" rows="1" value="${username}" name="username" id="name" onkeyup="return proverkaName(this);" class="intext" pattern="^[А-Яа-яЁёA-Za-z]+$" required></h4>
                                            <h4 class="htext">Телефон<input type="text" value="${userPhone}" name="phone" class="intext" id="number" onkeyup="return proverkaNumber(this);" placeholder="+79332221100" pattern="(\+7)+[0-9]{10}" required></h4>
                                            <h4 class="htext">Полных лет<textarea cols="30" rows="1" name="age" <#if userAge??> value="${userAge}" </#if> id="age" class="intext"
                                                                                  onkeyup="return proverkaAge(this);"></textarea></h4>
                                            <h4 class="htext">Пол<select name="gender">
                                                                    <option <#if userGender?? && userGender == "Man">selected</#if> value="Man" class="intext">Man</option>
                                                                    <option <#if userGender?? && userGender == "Woman">selected</#if> value="Woman" class="intext">Woman</option>
                                                                </select>
                                            </h4>
